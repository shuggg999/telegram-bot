...
- [ ] 6.1 Write `src/formatter/volume_alert.py`: `format_volume_alert(payload: dict) -> str` returning HTML message body matching the existing `data-service src/alerts/formatter.py` output (so users see no visible difference). Include emoji + symbol + tier + ratio + level
...
- [ ] 9.3 Each sub-probe wrapped with `_safe_subprobe()` (copy pattern from data-service)
...
- [ ] 11.2 Finalize docker-compose.yml (joins data-service network, env vars, port 8300)
...
- [ ] 12.3 cp .env.example .env; fill `TELEGRAM_BOT_TOKEN` (re-use existing bot token from data-service .env), `TELEGRAM_DEFAULT_CHAT_ID`, CH creds (read-only fine for audit table SELECT, write to telegram_audit)
...
## 13. Switchover from data-service Internal Notifier

> **Coordination point with volume-monitor**: the moment volume-monitor starts POSTing alerts to telegram-bot, alerts will be sent twice (data-service internal notifier + telegram-bot). De-dupe via `(symbol, level, hour_bucket)` key on Telegram side OR temporarily disable data-service notifier.

- [ ] 13.1 Decide switchover strategy with user (recommended: stop data-service internal notifier first, then enable volume-monitor → telegram-bot path)
- [ ] 13.2 If chosen "stop data-service notifier first": set `ALERTS_DRY_RUN=true` in data-service .env on jarvis and restart data-service
...
- [ ] 14.3 Trigger follow-up: data-service opens `cleanup-business-modules` change to delete `src/alerts/`
